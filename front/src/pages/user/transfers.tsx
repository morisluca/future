...
export default function TransfersPage() {
  ...
  }, [wireModalStep, currentProgressStep, wireProgressSteps.length]);
  ...
  const handleWireModalConfirm = async () => {
    if (!form.imfCode || !form.cotCode || !form.transferPin) {
      toast({ variant: "destructive", title: "Error", description: "IMF code, COT code, and Transfer PIN are required." });
      return;
    }
    ...
    try {
      ...
      onSuccess(pending ? "Transfer submitted — awaiting  approval." : "Transfer completed successfully.");
    } catch (error: any) {
      setWireModalStep("input");
      setCurrentProgressStep(0);
      setIsProcessing(false);
  ...
  const handleDeposit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSuccess(pending ? "Deposit submitted — awaiting approval." : "Deposit credited to your account.");
    } catch (err: any) {
  ...
  return (
    <DashboardLayout>
      <div className="max-w-2xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold font-display text-white mb-2">Move Money</h1>
          <p className="text-zinc-400 text-sm">Transfer, deposit, or withdraw funds from your accounts.</p>
        </div>
        ...
        <div className="flex p-1 bg-zinc-900 rounded-xl border border-zinc-800">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => { setActiveTab(tab.id); setIsTransferSubmitted(false); reset(); }}
        ...
        <div className="glass-card rounded-2xl p-6 border border-zinc-800">
          ...
          {activeTab === "transfer" && (
            <form onSubmit={handleTransfer} className="space-y-5">
              ...
              <Button type="submit" size="lg" className="w-full h-14 text-base mt-2" disabled={isPending || isTransferSubmitted || !user?.hasTransferPin}>
            ...
            <div className="space-y-6">
              ...
              {(depositInfo.deposit_bank_name || depositInfo.deposit_account_number) ? (
                ...
                <div className="bg-zinc-800/40 border border-zinc-700 rounded-xl p-5 text-center">
                  ...
                  <p className="text-sm text-zinc-500">Deposit bank details have not been configured by the bank yet.</p>
                </div>
              )}
            ...
            {wireModalStep === "input" && (
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-sm text-zinc-400">Enter your IMF code, COT code, and transfer PIN to proceed.</p>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {form.imfCode && (
                    <div className="animate-in fade-in">
                      <Field label="COT Code">
                        <Input className={inputCls} placeholder="COT code" value={form.cotCode} onChange={setField("cotCode")} />
                      </Field>
                    </div>
                  )}
                  {form.imfCode && form.cotCode && (
                    <div className="animate-in fade-in">
                      <Field label="Transfer PIN">
                ...
                {form.imfCode && form.cotCode && form.transferPin && (
              ...
              <div className="space-y-6 py-4">
                ...
                <div className="space-y-3">
                  {wireProgressSteps.slice(0, Math.min(currentProgressStep + 2, wireProgressSteps.length)).map((step, index) => (
                    <div key={index} className="flex items-center gap-3 animate-in fade-in">
                      ...
                      <span className={cn("text-sm font-medium", index < currentProgressStep ? "text-emerald-400" : index === currentProgressStep ? "text-blue-400" : "text-zinc-500")}>
                        {step}
                      </span>
                    </div>
                  ))}
                ...
                {currentProgressStep >= wireProgressSteps.length && (
                  <div className="space-y-3 pt-3 border-t border-zinc-800">
                    ...
                    <Button className="w-full h-12" onClick={() => setShowWireModal(false)}>
                      Close
                    </Button>
                  </div>
                )}
